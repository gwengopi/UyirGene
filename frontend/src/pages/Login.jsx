...
function Login() {
  ...
  // Where to go after login — supports both location objects (ProtectedRoute) and pathname strings
  const fromState = location.state?.from;
  const redirectAfterLogin = (() => {
    if (!fromState) return ROUTES.DASHBOARD;
    if (typeof fromState === 'string' && fromState.startsWith('/') && !fromState.startsWith('//')) return fromState;
    if (fromState?.pathname?.startsWith('/') && !fromState.pathname.startsWith('//')) return fromState.pathname;
  ...
  const handleSubmit = async (e) => {
    ...
    const validationErrors = validateForm(
      { email: { required: true, email: true, label: 'Email' }, password: { required: true, label: 'Password' } },
      formData
    );
  ...
  return (
    ...
    >
      ...
      <Container maxWidth={false} sx={{ maxWidth: 780, px: { xs: 2, sm: 3 }, position: 'relative', zIndex: 1 }}>
        <Paper
          ...
          >
            ...
            <Box component="form" onSubmit={handleSubmit} noValidate>
              ...
              <Box sx={{ textAlign: 'right', mb: 2 }}>
                <Link component={RouterLink} to={ROUTES.FORGOT_PASSWORD} variant="caption" sx={{ color: 'text.secondary' }}>
                  Forgot password?
                </Link>
              </Box>
              ...
              <Typography variant="caption" sx={{ display: 'block', textAlign: 'center', mt: 2.5, color: 'text.secondary' }}>
                ...
                <Link component={RouterLink} to={ROUTES.REGISTER} fontWeight={600} sx={{ color: isDarkMode ? '#90caf9' : 'rgb(79,102,114)' }}>
                  Sign up
                </Link>
              </Typography>
